...
async fn characters(api: &EnkaNetwork) -> Result<Value, String> {
    ...
    */
    ...
    let raw = api
        .assets("https://github.com/theBowja/genshin-db/raw/main/src/data/image/characters.json")
        .await?;
    ...
    if let Ok(json) = cache {
        if let Err(e) = api
            .assets_cache
            .set(local_path, &json, std::time::SystemTime::now())
            .await
        {
            println!("{}", e)
        }
...
pub(crate) async fn store(api: &EnkaNetwork) -> Result<StoreValue, String> {
    let (loc_json, namecards_json, characters_json) = futures::join!(
        _store(api, "loc.json"),
        _store(api, "namecards.json"),
        characters(api)
    );
    Ok(StoreValue {
        loc: loc_json?,
