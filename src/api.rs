...
use crate::{
    ...
    FightPropLocale, MemoryCache, RawUserData, UserData,
};
...
impl EnkaNetwork {
    ...
    pub async fn assets(&self, name: impl AsRef<str>) -> Result<Arc<Vec<u8>>, String> {
        ...
        let base_url = if local_path.starts_with("store/") {
            ...
            || local_path.starts_with("fonts/")
        {
            "https://enka.network/"
        } else if (local_path.starts_with("https://") || local_path.starts_with("http://"))
        ...
        match self.assets_cache.get(&local_path).await {
            ...
            Err(_) => {
                ...
                ioerror(
                    self.assets_cache
                        .set(&local_path, &contents, SystemTime::now())
                        .await,
                    format!("create cache assets {:?}", &local_path),
                )?;
