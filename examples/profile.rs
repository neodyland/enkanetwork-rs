...
use enkanetwork_rs::{Character, EnkaNetwork, IconData, NameCard, Reliquary, UserData, Weapon};
...
async fn create_user_data(
    ...
    match data {
        Ok(data) => {
            ...
            for id in avatar_list {
                create_char_cards.push(async {
                    if let Some(character) = data.character(*id) {
                        ...
                        let char_card =
                            character_card(&api, &character, &font, language.as_ref(), &icons)
                                .await;
                        char_card
                            .save(format!("./img/{}_card.png", &character_name))
                            .unwrap();
                    }
                });
            }
            ...
            futures::join!(
                create_profile_card,
                futures::future::join_all(create_char_cards)
            );
        }
        Err(e) => {
            println!("{}", e);
    ...
    let name_card = async {
        ...
        let profile_icon = async {
            match data.character(profile.profile_picture()) {
                Some(profile_character) => profile_character.image_icon(api).await,
                None => None,
            }
        };
...
async fn show_name_card_list(
    api: &EnkaNetwork,
    name_cards: &Vec<NameCard>,
) -> Option<DynamicImage> {
    ...
    if !name_card_images.is_empty() {
        ...
        let height =
            (name_card_images.len() as u32 / 3) * name_card_images.get(0).unwrap().height();
...
async fn character_card(
    ...
    let mut char_card = {
        ...
        let char_card = image::io::Reader::new(char_card)
            .with_guessed_format()
            .unwrap();
        ...
        image::imageops::resize(
            &char_card,
            (char_card.width() as f32 * 1.2) as u32,
            (char_card.height() as f32 * 1.2) as u32,
    ...
    if let Some(avater_image) = avater_image {
        ...
        let mut avater_image = avater_image
            .resize(w, h, image::imageops::Triangle)
            .into_rgba8();
    ...
    }
    {
        ...
        for p in fade.pixels_mut() {
            ...
            let alpha = if alpha > 100 { 100 } else { alpha as u8 };
            p.0 = [0, 0, 0, alpha];
            x_index += 1;
            if x_index >= width {
                x_index = 0;
    ...
    if friendship_level > 0 {
        let mut friendship = icons
            .image_color("Friendship.svg", 3f32, image::Rgba([90, 90, 90, 0]))
            .unwrap();
    ...
    let status_x = {
        ...
        let mut x_index = 0;
        for p in fade.pixels_mut() {
            let alpha = if x_index > 100 { 100 } else { x_index as u8 };
            p.0 = [0, 0, 0, alpha];
            x_index += 1;
            if x_index >= width {
                x_index = 0;
    ...
    .await;
    let set_name = render_reliquarys(
        api,
        &mut char_card,
        character.reliquarys(),
        icons,
        font,
        status_x + 530,
        20,
...
async fn render_reliquarys(
    ...
    let bg_img = {
        ...
        for px in img.pixels_mut() {
            let alpha = if i < 160 { i as u8 } else { 160 };
            i += 1;
            px.0 = [0, 0, 0, alpha];
            if i >= width {
                i = 0;
    ...
    for i in 0..reliquarys.len() {
        ...
        set_name.insert(
            set_name_hash,
            *set_name.get(&set_name_hash).unwrap_or(&0u8) + 1,
        );
...
fn render_fight_prop(
    ...
    let text = format!(
        "{}+{}",
        fight.base_hp.round() as i32,
        (fight.display_max_hp - fight.base_hp).round() as i32
    );
    ...
    if let Some(icon_image) = icons.image_color("FIGHT_PROP_HP.svg", 1.5f32, color_white) {
        image::imageops::overlay(
            char_card,
            &icon_image,
            fight_prop_x as i64 - 30i64,
            fight_prop_y as i64 + 8i64,
        );
    }
    ...
    let text = format!(
        "{}+{}",
        fight.base_attack.round() as i32,
        (fight.display_attack - fight.base_attack).round() as i32
    );
    let (x, y) = imageproc::drawing::text_size(subvalue_size, &font, &text);
    imageproc::drawing::draw_text_mut(
        char_card,
    ...
    let text = format!(
        "{}+{}",
        fight.base_defense.round() as i32,
        (fight.display_defense - fight.base_defense).round() as i32
    );
    let (x, y) = imageproc::drawing::text_size(subvalue_size, &font, &text);
    imageproc::drawing::draw_text_mut(
        char_card,
    ...
    if let Some(icon_image) =
        icons.image_color("FIGHT_PROP_ELEMENT_MASTERY.svg", 1.5f32, color_white)
    {
        image::imageops::overlay(
            char_card,
            &icon_image,
            fight_prop_x as i64 - 30i64,
            fight_prop_y as i64 + 13i64,
        );
    }
    ...
    if let Some(icon_image) =
        icons.image_color("FIGHT_PROP_CHARGE_EFFICIENCY.svg", 1.5f32, color_white)
    {
        image::imageops::overlay(
            char_card,
            &icon_image,
    ...
    for (element, dmg) in &fight.damage_bonus {
        ...
        let text = match api.get_store() {
            Ok(store) => element.attack_name(store, language),
            _ => "",
        };
        imageproc::drawing::draw_text_mut(
            char_card,
